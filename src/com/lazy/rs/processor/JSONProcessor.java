...
public class JSONProcessor {
	 ...
	 * @return JSON representation of ResultSet
	 * e.g. if ResultSet contains a row [first_name, last_name with values java, language] then
	 * returned JSON would be: {"first_name": "java", "last_name": "language"}
	 * 
	 * */
	public String resultSet2JSONString(ResultSet rs) {
		...
		try {
			...
			for(int i = 1; i <= meta.getColumnCount(); i++) {
				...
				json += "\"" + meta.getColumnName(i).toLowerCase() + "\":\"" + normalizeForJSON(value) + "\"";
			}
		} catch(Exception e) {
	 ...
	 * @param rs ResultSet to read from
	 * @return JSON representation of ResultSet
	 * e.g. if ResultSet contains two rows [first_name, last_name with 
	 * values (java, language) and (json, object)] then
	 * returned JSON would be: [{"first_name": "java", "last_name": "language"},
	 * {"first_name": "json", "last_name": "object"}]
	 * 
	 * */
	...
	private String normalizeForJSON(Object val) {
		...
		if(val != null) {
			normalizedString = val.toString()
									.replaceAll("\\\\", "\\\\\\\\") // Escapes backward slash 
